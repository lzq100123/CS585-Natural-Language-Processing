				...
				while(true)
				{
					...
					if(grammar.canParse(arr))
					{
						if((result = grammar.parse(arr)) != null)
						{
							...
							//flag to check if all parse results are processed
							boolean notEnd = true;
							//To store all parse results into list
							List<String[]> res = new ArrayList<>();
							while(notEnd){
								//sentence size, parse result and probability
								String[] str = {String.valueOf(arr.length),"","0.0"};
								str = result.recursivePrint(str);
								if(res.size() > 0 && res.get(res.size() - 1)[1].equals(str[1]))
									notEnd = false;
								else
									res.add(str);
							}
							String finalParse = res.get(0)[1];
							float prob = Float.parseFloat(res.get(0)[2]);
							//find the highest probability of parse result
							for(int i = 1;i < res.size();i++){
								if(prob < Float.parseFloat(res.get(i)[2])){
									finalParse = res.get(i)[1];
									prob = Float.parseFloat(res.get(i)[2]);
								}
							}
							//print the final result
							System.out.println(finalParse);
						}else{
